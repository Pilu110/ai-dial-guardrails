...
class PresidioStreamingPIIGuardrail:
    ...
    def __init__(self, buffer_size: int =100, safety_margin: int = 20):
        ...
        self.nlp_engine_provider = NlpEngineProvider(nlp_configuration=self.nlp_configuration)

        # 3. Create AnalyzerEngine, as `nlp_engine` crate engine by crated provider (will be used as obj var later)
        self.analyzer = AnalyzerEngine(nlp_engine=self.nlp_engine_provider.create_engine())
...
def main(use_presidio_guardrail = True):

    # 1. Create PresidioStreamingPIIGuardrail or StreamingPIIGuardrail
    if use_presidio_guardrail:
        guardrail = PresidioStreamingPIIGuardrail(buffer_size=200, safety_margin=40)
    else:
    ...
    while True:
        ...
        for chunk in llm_client.stream(messages):
            safe = guardrail.process_chunk(chunk.content)
            if safe:
                print(safe, end="", flush=True)
                response += safe

        safe = guardrail.finalize()
        if safe:
            print(safe, end="", flush=True)
            response += safe
...
if __name__ == "__main__":
    main(use_presidio_guardrail=False)
